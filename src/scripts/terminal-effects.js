...
function processTerminalCommand(command, outputElement) {
  ...
  switch(command.toLowerCase()) {
    ...
    case 'vault':
      response = `
        <div class="terminal-response">
          <div>VAULT-TEC TERMINAL ACCESS</div>
          <div>-----------------------------------------</div>
          <div>REDIRECTING TO VAULT SHELTER...</div>
        </div>
      `;
      setTimeout(() => {
...
function getProjectDetails(projectName) {
  const projects = {
    ...
    'vault shelter': `
      <div class="terminal-response">
        <div>PROJECT: VAULT SHELTER GAME</div>
        <div>-----------------------------------------</div>
        <div>STATUS: BETA</div>
        <div>DESCRIPTION: A Fallout-inspired Tamagotchi-like game with vault management, character development, and wasteland exploration. Features include resource management, room building, SPECIAL stats, and combat systems.</div>
        <div>TECHNOLOGIES: JavaScript, HTML5 Canvas, Solidity</div>
        <div><a href="/vault-shelter" class="terminal-link">PLAY GAME</a></div>
      </div>
    `,
    'blockchain tools': `
      <div class="terminal-response">
