...
export function generateSlug(text: string): string {
  if (typeof text !== 'string') {
    throw new Error(`generateSlug expects a string but received ${typeof text}: ${JSON.stringify(text)}`);
  }
...
/**
 * Generates the main blog landing page path
 * @returns The path to the blog index page
 */
export function getBlogIndexPath(): string {
  return '/blog';
}

/**
 * Generates the main services landing page path
 * @returns The path to the services index page
 */
export function getServicesIndexPath(): string {
  return '/services';
}
...
export function generateBlogTagPath(tag: string): string {
  if (typeof tag !== 'string') {
    ...
    if (typeof tag === 'object' && tag !== null && 'name' in tag) {
      ...
      throw new Error(`generateBlogTagPath expects a string but received ${typeof tag}: ${JSON.stringify(tag)}`);
    }
  }
 ...
 */
export function validatePath(path: string, type: 'service' | 'blog' | 'blog-category' | 'blog-tag'): boolean {
  const patterns = {
 ...
 */
export function extractSlugFromPath(path: string, type: 'service' | 'blog' | 'blog-category' | 'blog-tag'): string | null {
  const patterns = {
    service: /^\/services\/(.+)$/,
    blog: /^\/blog\/(.+)$/,
 ...
 */
export function generateCanonicalUrl(path: string, baseUrl: string = process.env.SITE_URL || ''): string {
 ...
 */
export function isActivePath(currentPath: string, linkPath: string, exact: boolean = false): boolean {
  if (exact) {
...
export function generatePaginationPaths(basePath: string, currentPage: number, totalPages: number) {
  ...
  return {
    ...
    previous: currentPage > 1 ? (currentPage === 2 ? basePathClean : `${basePathClean}/page/${currentPage - 1}`) : null,
