...
export function initializeHeader() {
  ...
  if (mobileMenu) {
    ...
    const topUtilityClasses = Array.from(mobileMenu.classList).filter(
      (cls) => cls.startsWith("top-") && !cls.includes("["),
    );
    ...
    const topStyleObserver = new MutationObserver((mutations) => {
      for (const m of mutations) {
        if (m.type === "attributes" && m.attributeName === "style") {
          ...
          const strayTopClasses = Array.from(mobileMenu.classList).filter(
            (cls) => cls.startsWith("top-") && !cls.includes("["),
          );
    ...
    topStyleObserver.observe(mobileMenu, {
      attributes: true,
