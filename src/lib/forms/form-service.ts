...
export async function submitContactForm(
  ...
  return {
    success: true,
    message:
      "Your email client should open with a pre-filled message. Please send it to complete your request.",
  };
}
...
export async function submitNewsletterSubscription(
  ...
  if (config.formspreeEndpoint) {
    try {
      const newsletterEndpoint = config.formspreeEndpoint.replace(
        "/contact",
        "/newsletter",
      );
  ...
  return {
    success: false,
    message:
      "Newsletter subscription is currently unavailable. Please contact us directly.",
