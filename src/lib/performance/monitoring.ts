...
export class PerformanceMonitor {
  ...
  public async sendMetrics(): Promise<void> {
    const payload = {
      ...
      customMetrics: Array.from(this.customMetrics.entries()).map(([name, values]) => ({
        name,
        values
      })),
      timestamp: Date.now(),
    ...
    if (process.env.NODE_ENV === 'development') {
      console.warn('📤 Performance metrics (would be sent to analytics):', payload);
      return;
    }
  ...
  public trackError(error: Error, context?: string): void {
    ...
    import('../error-handling').then(({ trackError }) => {
   ...
   */
  public measureBetweenMarks(name: string, startMark: string, endMark: string): void {
  ...
  public getPerformanceReport(): {
    ...
    if (this.metrics.largestContentfulPaint) {
      if (this.metrics.largestContentfulPaint > 4000) {
        recommendations.push('LCP is too high (>4s). Optimize images and reduce render-blocking resources.');
        score -= 20;
      } else if (this.metrics.largestContentfulPaint > 2500) {
        recommendations.push('LCP needs improvement (>2.5s). Consider lazy loading and image optimization.');
        score -= 10;
      }
    }
    ...
    if (this.metrics.firstInputDelay) {
      if (this.metrics.firstInputDelay > 300) {
        recommendations.push('FID is too high (>300ms). Reduce JavaScript execution time.');
        score -= 20;
      } else if (this.metrics.firstInputDelay > 100) {
        recommendations.push('FID needs improvement (>100ms). Optimize JavaScript and use code splitting.');
        score -= 10;
      }
    }
    ...
    if (this.metrics.cumulativeLayoutShift) {
      if (this.metrics.cumulativeLayoutShift > 0.25) {
        recommendations.push('CLS is too high (>0.25). Add size attributes to images and avoid dynamic content insertion.');
        score -= 20;
      } else if (this.metrics.cumulativeLayoutShift > 0.1) {
        recommendations.push('CLS needs improvement (>0.1). Reserve space for dynamic content.');
        score -= 10;
      }
    }
