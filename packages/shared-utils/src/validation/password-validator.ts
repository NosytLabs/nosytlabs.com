...
const COMMON_WORDS = [
  'password', 'admin', 'user', 'login', 'test', 'demo', 'sample',
  'example', 'temp', 'temporary', 'new', 'old', 'current', 'previous',
  'first', 'last', 'name', 'email', 'phone', 'address', 'account',
  'summer', 'winter', 'spring', 'fall', 'january', 'february', 'march',
  'april', 'may', 'june', 'july', 'august', 'september', 'october',
  'november', 'december'
];

const KEYBOARD_PATTERNS = [
  'qwerty', 'qwer', 'asdfgh', 'asdf', 'zxcvbn', 'zxcv',
  '123456', '12345', '1234', '234567', '23456', '2345',
  '345678', '34567', '3456', '456789', '45678', '4567',
  '567890', '56789', '5678'
];
...
export function validatePasswordStrength(password: string | null | undefined, options: PasswordValidationOptions = {}): PasswordStrengthResult {
  const {
    minLength = 8,
    maxLength = 128,
  ...
  const extraLengthBonus = password.length > 20 ? Math.min(password.length - 20, 20) * 0.25 : 0; // up to +5
  ...
  if (checkCommonPatterns) {
    ...
    const complexityHigh = password.length >= 20 && hasUppercase && hasLowercase && hasDigits && hasSpecial;
    const penaltyMultiplier = password.length >= 40 ? 0.25 : (complexityHigh ? 0.5 : 1);

    const anchoredCommon = COMMON_PATTERNS.some(pattern => pattern.test(password));
    const passwordPlusDigits = /password\d+/.test(lowerPassword);
    const hasCommonPattern = anchoredCommon || passwordPlusDigits || COMMON_WORDS.some(word => containsWordLike(normalizedLower, word));
    const hasKeyboardPattern = KEYBOARD_PATTERNS.some(pattern => lowerPassword.includes(pattern));
    const hasDatePattern = DATE_PATTERNS.some(pattern => pattern.test(password));
    const hasRepetitiveChars = /(.)\1{2,}/.test(password);
    const hasSequentialChars = /(0123|1234|2345|3456|4567|5678|6789|7890|abcd|bcde|cdef|defg|efgh|fghi|ghij|hijk|ijkl|jklm|klmn|lmno|mnop|nopq|opqr|pqrs|qrst|rstu|stuv|tuvw|uvwx|vwxy|wxyz)/i.test(password);
    const hasCommonSubstitutionChars = /[@$!013457]/.test(lowerPassword);
    const hasCommonSubstitutions = hasCommonSubstitutionChars && normalizedLower !== lowerPassword && COMMON_WORDS.some(word => containsWordLike(normalizedLower, word));
    
    if (hasCommonPattern) {
      feedback.push('common pattern');
      score = Math.max(0, score - Math.round((anchoredCommon ? 25 : 8) * penaltyMultiplier));
    }
  ...
  if (checkDictionaryWords) {
    ...
    const hasDictionaryWord = COMMON_WORDS.some(word => containsWordLike(lowerPassword, word) || containsWordLike(normalizedLower, word));
...
export function getPasswordStrengthColor(strength: PasswordStrengthResult['strength']): string {
  switch (strength) {
    ...
    case 'weak': return 'hsl(185, 75%, 50%)';
...
export function validatePassword(password: string, options?: PasswordValidationOptions): PasswordValidationResult {
  ...
  return {
    ...
    strength,
    errors: result.feedback.filter(f => f !== 'Password meets all security requirements')
  };
}

export function getPasswordStrengthText(strength: PasswordStrengthResult['strength']): string {
  switch (strength) {
