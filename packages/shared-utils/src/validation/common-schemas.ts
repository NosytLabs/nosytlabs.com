...
export const emailSchema = z
  ...
  .pipe(z.string().email('Invalid email format').max(255, 'Email must be 255 characters or less'));
...
export const imageFileSchema = fileSchema.extend({
  ...
  size: z.number().int().max(10 * 1024 * 1024, 'Image size must be 10MB or less') // 10MB
});

export const videoFileSchema = fileSchema.extend({
  type: z.string().regex(/^video\//, 'File must be a video'),
  size: z.number().int().max(100 * 1024 * 1024, 'Video size must be 100MB or less') // 100MB
});
...
export const apiResponseSchema = <T extends z.ZodTypeAny>(dataSchema: T) => {
  return z.object({
    ...
    timestamp: z.string().datetime().transform(date => new Date(date)).optional().default(() => new Date().toISOString())
  });
};
