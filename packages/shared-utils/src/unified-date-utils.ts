...
export function isValidDate(
  date: unknown,
  options?: { min?: Date; max?: Date; validator?: (date: Date) => boolean },
): date is Date {
...
export function parseRelativeTime(input: string): Date | null {
  ...
  const match = trimmed.match(
    /^(\d+)\s*(millisecond|milliseconds|second|seconds|minute|minutes|hour|hours|day|days|week|weeks|month|months|year|years)\s*ago$/,
  );
  if (match) {
...
export function formatDate(
  ...
  const effectiveCustomFormat =
    customFormat ?? (isTokenFormat ? (format as string) : undefined);
...
export function formatDuration(
  duration: number | DurationObject | null | undefined,
...
export function addDuration(
  ...
  if (typeof duration === "string") {
    ...
    if (duration.seconds)
      result.setTime(result.getTime() + duration.seconds * 1000);
    if (duration.minutes)
      result.setTime(result.getTime() + duration.minutes * 60 * 1000);
    if (duration.hours)
      result.setTime(result.getTime() + duration.hours * 60 * 60 * 1000);
    if (duration.days)
      result.setTime(result.getTime() + duration.days * 24 * 60 * 60 * 1000);
...
export function getTimeAgo(
  ...
  if (diffMs < 0) {
    ...
    if (futureDiffMinutes < 60)
      return `${futureDiffMinutes} minute${futureDiffMinutes === 1 ? "" : "s"} from now`;

    const futureDiffHours = Math.floor(futureDiffMinutes / 60);
    if (futureDiffHours < 24)
      return `${futureDiffHours} hour${futureDiffHours === 1 ? "" : "s"} from now`;

    const futureDiffDays = Math.floor(futureDiffHours / 24);
    if (futureDiffDays < 30)
      return `${futureDiffDays} day${futureDiffDays === 1 ? "" : "s"} from now`;
  }
